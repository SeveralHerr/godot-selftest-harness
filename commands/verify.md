...
| (c) Only `static func`s or `const` tables that existing unit tests cover — **or an instance method whose changed call site is already driven by a hosted-scene test** in the headless suite (a `test_*` that `instantiate_scene`s the real scene and drives the real path; name it) | **Headless-only** | Phase 1 (name gate + import gate + lint + tests). Skip runtime; the Phase 6 entry must **name which tests** stood in for runtime, and the row records `tier: headless-only` with `runtime: {"skipped": "<the test that covers it>"}` — a reasoned skip, not a run that had no runtime (plant G-064). Launching would re-drive the same code with a renderer attached. |
...
| (f) Only **project-owned tooling outside `res://`** — a `tools/*.py` checker the project ships and runs, a script no GDScript imports | **Tooling-only** | No Godot phase can speak to it (a new Python file that nothing in `res://` imports cannot break GDScript, and the harness gates would only confirm that). Run that tool's own tests / self-check (`python -m unittest discover -s tools`, or the tool's `--self-check`) and record the row with `"tier": "tooling"`; `found` names what the tool caught. (plant G-060, 2nd sighting: a `tools/run_json_check.py` plus two `.md`s fit no row, and ~40 s of harness gates "told me nothing, which is what overkill is for".) |
| Anything else — instance methods, signals, scenes, exports, node paths, config | **Full run** | All phases. |

**Record the tier on the row.** Whatever tier ran, `run.json` carries `"tier": "<full|headless-only|lint-only|nothing|experiment|tooling>"` — `stats` counts rows by it, and it is the field that lets the ledger say "how often is the full run the right call" instead of only "how often was it worth it". (Rows before 0.50.0 dropped the key silently; the 0.48.0+ `read K of N keys` line names it.)
...
| `find-nodes --class X --where prop=value` | Identify a node by what it *is*, instead of one `get-state` round trip per auto-named sibling. `--class` / `--group` / `--method` pick the population, `--where NAME=VALUE` narrows it (repeatable, dotted paths allowed), `--property NAME` reports a value per hit (an unresolvable one prints its reason, not `null`), `--call METHOD` reads a zero-arg getter per hit, `--root` confines the search. A node lacking the property is a non-match, never an error, so a predicate is safe across a mixed subtree |
| `step-time --seconds N [--then-pause]` | Advance ~N game-seconds with `time_scale` pinned to 1.0 — for sampling a tween at a chosen moment instead of guessing with `set-game-speed` + sleeps. Physics time is exact; process-driven tweens (the `Tween` default) land within ~1 frame, so compare the returned `process_seconds` rather than assuming. `--then-pause` freezes the tree the moment the step lands, so the read that follows carries no ambient drift (`unpause` resumes) |
| `contained-in --node PATH --within PATH` | Is a Control's screen box inside a panel/card it visibly sits on? Exit 1 with the per-side overhang. `findings` reports the sibling-panel case as `ui_escapes_panel`; use this for a specific pair, or a non-sibling one |
| `project-settings --filter PREFIX` / `--name KEY` | `ProjectSettings` as the running game sees them — the one read that tells a `project.godot` value that landed from one that did not (a typo'd key, an editor overwrite). `get-state` cannot: `ProjectSettings` is not a node |
...
| `batch --json-items '[{"action":"set-state","args":{...}},{"action":"get-state","args":{...}}]'` | Several primitives in ONE round trip - a set/read/read that would cost three bus calls costs one; every item's envelope comes back in order and `failed` names the indices. Use it for a tight read-modify-read; `--stop-on-error` when a later item depends on an earlier one |
...
The command prints which changed files the run reached and names the ones it did not, with the same worktree/branch split and the same `implicit` / `alias` / excused-test annotations described above. **Reach is file-level** — the line says `[file-level: loaded, not lines-executed]` — and `reach` lists the changed functions inside each reached file (advisory, from `git diff -U0`); when the change is a guard clause, an early return or a `_process()` body, a reached file is not evidence the change ran — read a state the changed lines would have moved (`get-state`) or call the function (`run-method`) and say which you did. **Carry the worktree reach line into the Pass/Fail Summary verbatim.** A green run on a file nothing touched is a statement about the diff, not about the running game, and it is the one failure this workflow cannot otherwise see — the summary says "all checks passed" either way.
